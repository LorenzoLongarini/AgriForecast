...
class CallbackHandler:
    def __init__(self, callbacks: List[Callback] = [SystemMonitorCallback()], call_every_ms: int = 100):
    ...
    def collect(self, key, reset: bool = True) -> Dict[str, Any]:
        data = {"time": self.time}
        for callback in self.callbacks:
            data.update(callback.collect(reset=reset))
        data = {f'{key}_{k}': v  for k, v in data.items()}     
        return data
